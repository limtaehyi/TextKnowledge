...
* 시스템 콜은 사용자가 커널 레벨의 통신이며 사용자가 직접 하드웨어에 접근 할 수 없기 때문에 커널에 시스템 콜을 통해 사용하는 것이다.
...
* open : int fd = open("/home/flag", O\_RDONLY, Null)
...
* 두번째 인자 종류 : O\_RDONLY(읽기 전용), O\_WRONLY(쓰기 전용), 0\_RDWR(읽고 쓰기), 0\_APPEND(기존 내용에 이어 확장), 0\_CREAT(파일이 존재하지 않을 때 파일 생성)
* 세번째 인자 종류 : RWX 즉 777
* 어셈블리어
...
&#x20;정보 유지 -getpid(), alarm(), sleep(), 시간과 날짜의 설정과 획득(time), 시스템 데이터의 설정과 획득(date), 프로세스 파일, 장치 속성의 획득 및 설정

<br>

&#x20;통신

* pipe(), shm\_open(), mmap(), 통신 연결의 생성, 제거, 메시지의 송신, 수신, 상태 정보 전달, 원격 장치의 부착 및 분리
...
* https://elixir.bootlin.com/linux/v3.14/source/arch/x86/syscalls/syscall\_64.tbl
