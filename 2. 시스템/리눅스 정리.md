...
* 리눅스는 윈도우, 유닉스, 안드로이드, ios와 같은 os이다.
...
* 777 r(read) : 4 w(write) : 2 s(set) : 1
...
* 프린트 관련 명령어 : lpd, lpc, lpq, lpr, lprm, lpstat lpd : 프린터 데몬 lpc : 프린트 제어 lpq : 기본 프린터의 큐확인 lpr : 파일 내용 출력 lprm : 프린터 큐 작업 삭제 lpstat : 프린터 상태 확인
* 프린터 관리 : cups
* 이미지 스캔 : scanimag
