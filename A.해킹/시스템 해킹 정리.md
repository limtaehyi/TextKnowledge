...
* heap : 자료구조의 힙은 tree 구조를 지니고 있다. 하지만 메모리의 힙은 여러개의 chunk단위로 나뉘며 할당되면 header(prev\_size, size), data로 해제되면 header(prev\_size, size), fd(이전chunk), bk(다음chunk)로 이루어져 있다. 여기에서 힙 오버플로우는 스택과 다르게 프로그램의 함수 포인터를 조작하여 공격을 한다.
* 예방 방법은 안전한 함수 사용, 컴파일러 및 런타임 운영체제 보호 기법 활용 등이 있다.
...
* 예방 방법은 출력함수 사용시 입력 받은 변수를 무조건 포맷 스트링 인자로 전달 금지, 안전한 함수 사용, 입력 값 검증 및 필터링 등이 있다.
...
* 예방 방법은 heap 할당 하고 난후 free를 했을 때 사용한 포인터 변수를 null로 덮어쓰기 생활화, 스마트 포인터 사용, 가비지 컬렉션 사용하기 등이 있다.

<br>

&#x20;RaCe Condition

* 자원 경쟁으로 cpu의 매우 빠른 처리속도로 인해 발생하는 취약점으로 쓰레딩 프로그램으로 확인해 볼 수 있다.
* 포너블에서는 임시파일 생성과 삭제로 발생 가능하고 웹에서는 뮤텍스 미적용으로 발생할 수 있다.
* 예방 방법은 뮤텍스 사용 및 정확한 범위 지정, 요청 주기 제한, 임시파일 사용 제한 등이 있다.

<br>

&#x20;Return To Csu

* \_\_libc\_csu\_init() 함수 내의 일부 코드를 gadget으로 사용하는 기법으로 rop가 사용 가능하지만 gadget이 부족한 경우 사용한다.
* 예방 방법은
...
* 할당된 메모리 정보들은 malloc\_chunk의 구조체로 pre\_size, size, fd, bk, fd\_next\_size, bk\_nextsize가 있다.
* pre\_size는 이전 chunk가 해제되면 설정되는 값, size는 현재 chunk 크기, fd는 이전 chunk의 주소값, bk는 다음 chunk의 주소값이다. fd, bk는 unsorted chunk list에 사용되는 값이며 chunk가 동적 할당되고 해제될때 unsorted chunk list에 들어간다.
...
* 스택 프레임에서 ret 값을 global ret에 복사한다. 스택 사용이 끝난 후 ret 시 global ret와 비교하여 값이 같으면 ret하고 다르면 프로그램 강제 종료한다.
