...
이것은 모바일 앱과 엔드 포인트 간의 상호 인증 기능을 파괴한다. 따라서 모바일 앱은 TLS프로시를 통한 man-in-the-middle attack에 취약한다.
-약한 핸드쉐이크 협상 : 클라이언트가 약한 암호 모음을 사용하기 위해 서버와 협상을 성공적으로 수행한다. 약한 암호 모음은 공격자가 쉽게 복호화할 수 있는 취약한 암호를 발생시킨다. 이것은 모바일 앱과 엔드포인트 간 채널 기밀성을 위태롭게 한다. 
